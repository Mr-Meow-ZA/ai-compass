...
render(){ local hash="$1" out="$2" w="$3" h="$4"; "$CHROME" --headless --no-sandbox --disable-gpu --hide-scrollbars --window-size="${w},${h}" --virtual-time-budget=12000 --dump-dom "http://127.0.0.1:4173/${hash}" >"$out"; }
shot(){ local hash="$1" out="$2" w="$3" h="$4"; "$CHROME" --headless --no-sandbox --disable-gpu --hide-scrollbars --window-size="${w},${h}" --virtual-time-budget=12000 --screenshot="$out" "http://127.0.0.1:4173/${hash}" >/dev/null 2>&1; }

render '#community' "$TMP/community.html" 1440 2100
shot '#community' "$ROOT/visual-smoke-community.png" 1440 2100
cp "$TMP/community.html" "$ROOT/visual-smoke-community.html"

render '#community/profile' "$TMP/profile.html" 1200 1400
shot '#community/profile' "$ROOT/visual-smoke-community-profile.png" 1200 1400
cp "$TMP/profile.html" "$ROOT/visual-smoke-community-profile.html"

render '#community/category/agents-orchestration' "$TMP/category.html" 1440 1600
shot '#community/category/agents-orchestration' "$ROOT/visual-smoke-community-category.png" 1440 1600
cp "$TMP/category.html" "$ROOT/visual-smoke-community-category.html"

render '#community' "$TMP/mobile.html" 390 1900
shot '#community' "$ROOT/visual-smoke-community-mobile.png" 390 1900
cp "$TMP/mobile.html" "$ROOT/visual-smoke-community-mobile.html"

count(){ grep -o "$1" "$2" | wc -l | tr -d ' ' || true; }
CATEGORIES="$(count 'class="community-category-card' "$TMP/community.html")"
MOBILE_CATEGORIES="$(count 'class="community-category-card' "$TMP/mobile.html")"
echo "Community rendered counts: categories=$CATEGORIES mobile-categories=$MOBILE_CATEGORIES"

for file in community profile category mobile; do
...
echo 'Community public-reader rendered smoke OK.'
