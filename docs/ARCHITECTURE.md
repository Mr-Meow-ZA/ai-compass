...
The current site is a static single-page application:

- `index.html` loads the application.
- `data.js` exposes structured content through browser globals.
- `app.js` provides hash routing, rendering, search, filters, saved items and article interactions.
...
Saved items and some community or editorial interactions use browser-local storage. They are not shared between users or devices.
...
That was a recovery workaround, not the target architecture. New development should happen only in this repository. The Vercel project should be connected directly to `Mr-Meow-ZA/ai-compass` after the imported source is verified.
...
- Separate content data by type.
- Add link and schema validation.
...
- Add an editorial status and change-history model.
